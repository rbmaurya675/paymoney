function showListOrder3(list_orders, x) {
    ...
    $(`.game-list .con-box:eq(${x}) #trx-data-list`).prepend(htmls);
    $(`.game-list .con-box:eq(${x}) #trx-data-list .c-tc`).last().remove();
  }
  var socket = io();
  var pageno = 0;
  var limit = 10;
  ...
  socket.on("data-server-trx", function (msg) {
    ...
    setTimeout(() => {
      ...
      $.ajax({
        ...
        //  let /api/webapi/GetNoaverageEmerdListtrxpopdata = response.data.gamessinglelist;
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          $(".time-box .info .table4").html(response.table4);
          $(".time-box .info .table5").html(response.table5);
  ...
  function showListOrder(list_orders, x) {
    ...
    let htmls = "";
    let result = list_orders.map((order) => {
      // Extract the last 5 characters of the hash
      let shortHash = order.hash.slice(-5);
      
      // Extract only the time part from the full date-time string
      let timeOnly = order.time.substring(11);
      let periodStr = order.period.toString();  // Ensure it's a string
      let maskedPeriod = `${periodStr.slice(0, 3)}**${periodStr.slice(-4)}`;
      let bigsmallDisplay = order.bigsmall === "big" ? "B" : order.bigsmall === "small" ? "S" : "";

      return (htmls += `
        <div data-v-a9660e98="" class="c-tc item van-row" style="display: flex; justify-content: space-between; padding: 8px 0;">
            <div data-v-a9660e98="" class="van-col van-col--5" style="padding-right: 10px; padding-left: 10px; color:black; padding-top: 25px;">
                <div data-v-a9660e98="" class="c-tc goItem">${maskedPeriod}</div>
            </div>
            <div data-v-a9660e98="" class="van-col van-col--5" style="padding-right: 10px; padding-left: 10px; color:black;">
                <div data-v-a9660e98="" class="c-tc goItem">
                <img src="/images/a.png" alt="Clickable Image" style="cursor: pointer;" style="width:35%" onclick="openUrl('https://tronscan.org/#/block/${order.block}')" />${order.block}</div>
            </div>
            <div data-v-a9660e98="" class="van-col van-col--5" style="padding-right: 10px; padding-left: 10px; color:black; padding-top: 25px;">
                <div data-v-a9660e98="" class="c-tc goItem">${timeOnly}</div>
            </div>
            <div data-v-a9660e98="" class="van-col van-col--4" style="padding-right: 10px; padding-left: 10px; color:black; padding-top: 25px;">
                <div data-v-a9660e98="" class="c-tc goItem">${shortHash}</div>
            </div>
            <div data-v-a9660e98="" class="van-col van-col--4" style="padding-right: 10px; padding-left: 10px; color:black; padding-top: 25px;">
                <div data-v-a9660e98="" class="c-tc goItem">${order.result}</div>
            </div>
            <div data-v-a9660e98="" class="van-col van-col--1" style="padding-right: 10px; padding-left: 10px; color:black; padding-top: 25px;">
            <div data-v-a9660e98="" class="goItem c-row c-tc c-row-center">
                <div data-v-a9660e98="" class="c-tc c-row box c-row-center">
                    <span data-v-a9660e98="" class="li ${order.amount % 2 == 0 ? "red" : "green"}"></span>
                    ${order.amount == 0 || order.amount == 5 ? '<span data-v-a9660e98="" class="li violet"></span>' : ""}
                </div>
                <div class="bigsmall-display">${bigsmallDisplay}</div> <!-- Show only "B" or "S" -->
            </div>
        </div>
    ...
    $(`.game-list .con-box:eq(${x}) #trx-data-list`).html(htmls);
  ...
  /*
      setInterval(function () {
        ...
        if (seconds1 == 5 && seconds2 <= 9) {
          ...
          $(".box .mark-box1 ").css("display", "block");
        } else {
          $(".box .mark-box1 ").css("display", "block");
        }
      }, 0);
  ...
  $(`.game-betting .box .item .img .van-image img`).attr('src', '/images/clock.png');
  $(`.game-betting .box .item:eq(${selectPageTime - 1}) .img .van-image img`).attr('src', '/images/clock-red.png');
  
  
  
  
  // Loss popup section JS
  function performActions(actions, popup, index) {
    if (index >= actions.length) {
        index = 0; 
    }
  
    setTimeout(function() {
        popup.style.display = actions[index].display;
        performActions(actions, popup, index + 1);
    }, actions[index].delay);
  }
  ...
  window.addEventListener("load", function() {
    ...
    var actions = [
        { display: "none", delay: 0 },  
        { display: "block", delay: 50000 },
        { display: "none", delay: 5000 } 
    ];
  
    performActions(actions, popup, 0);
  });